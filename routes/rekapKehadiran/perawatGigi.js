...
router.post("/cek", async function (req, res, next) {
  ...
  const stringQuery = `SELECT * FROM rekap_hadir_perawat_gigi WHERE bulan = ? AND tahun = ? AND cabang = ?`;
...
router.post("/delete", async function (req, res, next) {
  ...
  const stringQuery = `DELETE FROM rekap_hadir_perawat_gigi WHERE bulan = ? AND tahun = ? AND cabang = ? `;
  ...
  try {
    ...
    await Promise.all(
      result.map(async (item) => {
        const insertQuery = `INSERT INTO rekap_hadir_perawat_gigi 
      (tanggal, bulan, tahun, nama_shift, jam_masuk, jam_keluar, nama_perawat,nama_pengganti,nama_petugas, jbtn,cabang, nominal_shift, barcode, servicedoerid,denda_telat, denda_pulang_cepat,telat, pulang_cepat,total,total_jam,keterangan, createdAt, updatedAt)
      VALUES (?, ?, ?, ?, ?, ?, ?,?, ?,?,?, ?, ?, ?,?,?,?,?,?,? ?,?,?)`;
          ...
          await new Promise((resolve, reject) => {
            connection.query(insertQuery, values, (error, results) => {
              if (error) {
                ...
                console.log(
                  "Data berhasil disisipkan ke tabel rekap_hadir_perawat_gigi"
                );
                resolve();
              }
            });
