...
router.post("/nominal", function (req, res, next) {
  ...
  tanggalRange.forEach((tgl) => {
    ...
    (pegawai.jbtn = 'dr' OR pegawai.jbtn = 'drg');`;
          ...
          querySelectInsentif.forEach((query, index) => {
            connection.query(query, function (error, results, fields) {
              ...
              if (processedQueries === querySelectInsentif.length) {
                ...
                const addQuery =
                  "insert into rekap_insentif_shift (tanggal, bulan, tahun, nama_shift, nama_dokter, jbtn, insentif, nominal_shift, total_gaji, kekurangan_garansi_fee, garansi_fee, barcode, denda_telat, nama_dokter_pengganti, createdAt) values ('" +
                  tanggal +
                  "','" +
                  bulan +
                  "','" +
...
// Insentif Pengganti
router.post("/pengganti", function (req, res, next) {
  const { bulan, tahun } = req.body;

  const querySearch = `SELECT * FROM rekap_insentif_shift WHERE tahun='${tahun}' AND bulan='${bulan}' AND nama_dokter_pengganti != ''`;
  ...
  connection.query(querySearch, (error, result) => {
    ...
    console.log("pengganti: ", querySearch);
    res.json(result);
  });
});
...
// select sum(rawat_jl_pr.biaya_rawat) as total from reg_periksa inner join rawat_jl_pr on rawat_jl_pr.no_rawat=reg_periksa.no_rawat inner join penjab on reg_periksa.kd_pj=penjab.kd_pj where reg_periksa.tgl_registrasi = '2024-04-19' and rawat_jl_pr.nip='JP000025' and reg_periksa.jam_reg BETWEEN '13:00:00' and '18:00:00'

// select sum(rawat_inap_pr.biaya_rawat) as total from rawat_inap_pr inner join reg_periksa on rawat_inap_pr.no_rawat=reg_periksa.no_rawat inner join penjab on reg_periksa.kd_pj=penjab.kd_pj where reg_periksa.tgl_registrasi = '2024-04-19' and rawat_inap_pr.nip='JP000025' and reg_periksa.jam_reg BETWEEN '13:00:00' and '18:00:00'

// select sum(periksa_lab.tarif_tindakan_petugas) as total from periksa_lab inner join reg_periksa on periksa_lab.no_rawat=reg_periksa.no_rawat inner join penjab on reg_periksa.kd_pj=penjab.kd_pj where reg_periksa.tgl_registrasi = '2024-04-19' and periksa_lab.nip='${nik}' and reg_periksa.jam_reg BETWEEN '13:00:00' and '18:00:00'
